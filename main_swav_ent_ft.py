...
def train(train_loader, model, optimizer, epoch, lr_schedule, queue, ce_estimator):
    ...
    for it, inputs in enumerate(train_loader):
        ...
        for i, crop_id in enumerate(args.crops_for_assign):
            ...
            with torch.no_grad():
                hypemb_std += (ce_estimator.get_std(
                        embedding[crop_id * bs: (crop_id + 1) * bs]
        ...
        emb_mean = 0
        for i, crop_id in enumerate(args.crops_for_assign):
            # get the unnormalized embeddings for a view
            ...
            # print(f"{i=}, {z_i.shape=}, {z_i.mean(dim=0).mean()=}, {z_i.std(dim=0).mean()=}")
            ...
            """
            don't standardize the embedding, use the standard normal CDF to put values between
            0 and 1, and then maximize marginal entropies and minimize off-diagonal covariance
            to push the distribution to be 0-mean, identity covariance gaussian.
            """
        ...
        ce_losses.update(ce_loss.item(), inputs[0].size(0))
