...
parser.add_argument("--cov_coeff", type=float, default=0.01,
                    help='Covariance regularization for joint entropy loss coefficient')
parser.add_argument("--reg_space", type=str, default="hypersphere",
                    choices=["hypersphere", "projector"],
                    help='where to apply VC regularization, after projector or after l2 normalization')
...
def main():
    ...
    vc_estimator = VCEstimator(args.batch_size * args.world_size, args.feat_dim)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # train the network
        scores, queue = train(train_loader, model, optimizer, epoch, lr_schedule, queue, vc_estimator)
...
def train(train_loader, model, optimizer, epoch, lr_schedule, queue, vc_estimator):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
    swav_losses = AverageMeter()
    vc_losses = AverageMeter()
    ...
    for it, inputs in enumerate(train_loader):
        ...
        # ============ vc loss ... ============
        # z is (B*n, 128) dimensional
        # we only need the first 2 views from z.
        std_loss = 0
        cov_loss = 0
        emb_std = 0
        emb_mean = 0
        for i, crop_id in enumerate(args.crops_for_assign):
            if args.reg_space == "hypersphere":
                # using l2 embeddings
                # get the hypersphere embeddings for a view
                emb_i = embedding[bs * crop_id: bs * (crop_id + 1)]
                # aggregate the batch of embeddings from other GPUs
                emb_i = torch.cat(FullGatherLayer.apply(emb_i), dim=0)
                # subtract off the mean first
                emb_i = emb_i - emb_i.mean(dim=0)
                # compute the losses
                std_loss += vc_estimator.get_std_loss(emb_i, hyper=True) / len(args.crops_for_assign)
                cov_loss += vc_estimator.get_cov_loss(emb_i)
            elif args.reg_space == "projector":
                # using pre-l2 embeddings
                z_i = z[bs * crop_id: bs * (crop_id + 1)]
                # aggregate the batch of embeddings from other GPUs
                z_i = torch.cat(FullGatherLayer.apply(z_i), dim=0)
                emb_std += vc_estimator.get_std(z_i).detach().mean() / len(args.crops_for_assign)
                emb_mean += vc_estimator.get_mean(z_i).detach().mean() / len(args.crops_for_assign)
                # subtract off the mean first
                z_i = z_i - z_i.mean(dim=0)
                # compute the losses
                std_loss += vc_estimator.get_std_loss(z_i, hyper=False) / len(args.crops_for_assign)
                cov_loss += vc_estimator.get_cov_loss(z_i)

        vc_loss = args.cov_coeff * cov_loss + args.std_coeff * std_loss

        # ============ final loss ... ============
        loss = swav_loss + vc_loss
        ...
        vc_losses.update(vc_loss.item(), inputs[0].size(0))
        ...
        if args.rank == 0 and it % 50 == 0:
            logger.info(
                ...
                "VC Loss {vc_loss.val:.4f} ({vc_loss.avg:.4f})\t"
...
class VCEstimator(object):
    ...
    def get_std_loss(self, x, hyper=True):
        std_x = torch.sqrt(x.var(dim=0) + 0.0001)
        std_tgt = 1.0
        if hyper:
            std_tgt /= math.sqrt(self.num_features)
        std_loss = torch.mean(F.relu(std_tgt - std_x))
        return std_loss
