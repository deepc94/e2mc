...
class ResNet(nn.Module):
    def __init__(
        ...
        # return unnormalized embeddings from projector if true
        self.return_unnorm_emb = return_unnorm_emb
    ...
    def forward_head(self, x):
        ...
        if self.l2norm:
            z = nn.functional.normalize(x, dim=1, p=2)

        if self.prototypes is not None:
            if self.return_unnorm_emb:
                return x, z, self.prototypes(z)
            else:
