...
parser.add_argument("--unif_weight", type=float, default=1.0,
...
def train(train_loader, model, optimizer, epoch, lr_schedule, queue, ce_estimator):
    ...
    for it, inputs in enumerate(train_loader):
        ...
        # ============ uniformity loss ... ============
        # embedding is (B*n, 128) dimensional, and we only need first 2 views.
        unif_loss = 0
        ## just for monitoring
        ent_loss = 0
        hypercov_loss = 0
        ...
        for i, crop_id in enumerate(args.crops_for_assign):
            ...
            sq_dists = F.pdist(emb_i, p=2).pow(2)
            unif_loss += sq_dists.mul(-args.kernel_width).exp().mean().log() / len(args.crops_for_assign)

            # ============ ce loss for monitoring ============
            z_i = z[bs * crop_id: bs * (crop_id + 1)]
            z_i = torch.cat(FullGatherLayer.apply(z_i), dim=0)
            emb_std += ce_estimator.get_std(z_i).detach().mean() / len(args.crops_for_assign)
            emb_mean += ce_estimator.get_mean(z_i).detach().mean() / len(args.crops_for_assign)
            ent_z_i, hypercov_z_i_loss = ce_estimator.get_ent_and_cov_loss(z_i)
            ent_loss += ent_z_i.mean() / len(args.crops_for_assign)
            hypercov_loss += hypercov_z_i_loss

        # ============ final loss ... ============
        loss = swav_loss + args.unif_weight * unif_loss
        ...
        unif_losses.update(unif_loss.item(), inputs[0].size(0))
        ...
        if args.rank == 0 and it % 50 == 0:
            logger.info(
                ...
                "Unif Loss {unif_loss.val:.4f} ({unif_loss.avg:.4f})\t"
